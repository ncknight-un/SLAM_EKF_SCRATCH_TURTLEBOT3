...
TEST_CASE("Forward Path", "[update_fk and compute_ik]") {
    ...
    turtlelib::DiffDrive diff_drive(1.0, 1.0);
    ...
    SECTION("Forward Kinematics for simple forward path") {
        ...
        diff_drive.update_fk(1.0, 1.0);
        // Check that the robot base transform is updated correctly:
        turtlelib::Transform2D q = diff_drive.get_q();
        REQUIRE_THAT(q.translation().x, WithinRel(1.0, EPS));   // The robot should have moved forward 0.2 meters in the x direction in its frame 
    ...
    SECTION("Inverse Kinematics for simple forward path") {
        ...
        REQUIRE_THAT(vel.v_lw, WithinRel(0.2, EPS));   // Right wheel should rotate at 20 rad/s
        REQUIRE_THAT(vel.v_rw, WithinRel(0.2, EPS));   // Left wheel should rotate at 20 rad/s
    }
};
...
TEST_CASE("Pure Rotation Path", "[update_fk and compute_ik]") {
    ...
    SECTION("Forward Kinematics for simple rotation path") {
        ...
        REQUIRE_THAT(q.translation().y, WithinRel(0.0, EPS));
        REQUIRE_THAT(q.rotation(), WithinRel(-0.2, EPS));  // The robot should have rotated -1 radians in place
    }
...
TEST_CASE("Arc Path", "[update_fk and compute_ik]") {
    ...
    SECTION("Forward Kinematics for simple arc path") {
        ...
        diff_drive.update_fk(1.0, 2.0);
        // Check that the robot base transform is updated correctly:
        turtlelib::Transform2D q = diff_drive.get_q();
        REQUIRE_THAT(q.translation().x, WithinRel(1.2622, EPS));
        REQUIRE_THAT(q.translation().y, WithinRel(-0.6896, EPS));
        REQUIRE_THAT(q.rotation(), WithinRel(1.0, EPS));
// The robot should have rotated -2 radians and moved forward along the path 1 meters
    }
    ...
    SECTION("Inverse Kinematics for simple arc path") { 
        ...
        // Check that the required wheel angular velocities are correct:
        REQUIRE_THAT(vel.v_lw, WithinRel(0.0, EPS));   // Right wheel should rotate at 90.0 rad/s
        REQUIRE_THAT(vel.v_rw, WithinRel(2.0, EPS));  // Left wheel should rotate at 110.0 rad/s
    }
};
