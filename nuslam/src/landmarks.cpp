      ...
      this->create_subscription<sensor_msgs::msg::LaserScan>("laser_scan_data", 10,
        [this](const sensor_msgs::msg::LaserScan::SharedPtr msg) {
          ...
          for (size_t i = 0; i < ranges_.size(); i++) {
            ...
            "There are currently " << clusters.size() << " clusters detected.");
          ...
          RCLCPP_DEBUG_STREAM(this->get_logger(),
        "There are currently " << clusters.size() <<
            " clusters detected after removing small clusters.");
          ...
          for (auto & cluster : clusters) {
            ...
            RCLCPP_DEBUG_STREAM(this->get_logger(),
          "Cluster has " << cluster_points.size() << " points, is_circle: " << is_circle <<
              ", radius: " << radius << " (min: " << min_obstacle_radius_ << ", max: " <<
